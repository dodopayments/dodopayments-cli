...
export async function handlePayments(
  ...
  if (subCommand === 'list') {
    ...
    try {
      ...
      console.log(
        'To view a payment, go to https://app.dodopayments.com/transactions/payments/{payment_id}',
      );
    } catch (e) {
      if (isDodoPaymentsAPIError(e)) {
        console.log('Failed to fetch payments:', e.error.message);
      } else {
        console.error(e);
      }
    }
    ...
    const payment_id = await input({
      ...
      validate: (e) =>
        e.startsWith('pay_') || 'Please enter a valid payment ID!',
    });
    try {
      ...
      console.log(
        `To view the payment, go to https://app.dodopayments.com/transactions/payments/${info.payment_id}`,
      );
    } catch (e) {
