...
export async function handleProducts(
  ...
  if (subCommand === 'list') {
    ...
    try {
      ...
      console.table(table);
      console.log(
        'To edit a product, go to https://app.dodopayments.com/products/edit?id={product_id}',
      );
    } catch (e) {
      if (isDodoPaymentsAPIError(e)) {
        console.log('Failed to fetch products:', e.error.message);
      } else {
        console.error(e);
      }
    }
    ...
    const product_id = await input({
      ...
      validate: (e) =>
        e.startsWith('pdt_') || 'Please enter a valid product ID!',
    });
    ...
    try {
      ...
      console.log(
        `To edit the product, go to https://app.dodopayments.com/products/edit?id=${info.product_id}`,
      );
    } catch (e) {
