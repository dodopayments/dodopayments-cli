...
func getDefaultRequestOptions(cmd *cli.Command) []option.RequestOption {
	opts := []option.RequestOption{
		option.WithHeader("User-Agent", fmt.Sprintf("DodoPayments/CLI %s", Version)),
...
func (f *fileReader) Get() interface{} {
	return f.String()
}
...
func unmarshalStdinWithFlags(cmd *cli.Command, flags map[string]string, target interface{}) error {
	var data []byte
	if isInputPiped() {
		var err error
