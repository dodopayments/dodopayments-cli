...
var checkoutSessionsCreate = requestflag.WithInnerFlags(cli.Command{
	...
	Flags: []cli.Flag{
		...
		&requestflag.Flag[string]{
			...
			Usage:    "Product collection ID for collection-based checkout flow",
			BodyPath: "product_collection_id",
		},
		&requestflag.Flag[string]{
			Name:     "return-url",
...
var checkoutSessionsPreview = requestflag.WithInnerFlags(cli.Command{
	Name:    "preview",
	Usage:   "Perform preview operation",
	Suggest: true,
	Flags: []cli.Flag{
		&requestflag.Flag[[]map[string]any]{
			Name:     "product-cart",
	...
	Action:          handleCheckoutSessionsPreview,
	HideHelpCommand: true,
}, map[string][]requestflag.HasOuterFlag{
	"product-cart": {
		&requestflag.InnerFlag[string]{
...
func handleCheckoutSessionsPreview(ctx context.Context, cmd *cli.Command) error {
	client := dodopayments.NewClient(getDefaultRequestOptions(cmd)...)
	unusedArgs := cmd.Args().Slice()
	...
	params := dodopayments.CheckoutSessionPreviewParams{}
	...
	var res []byte
	options = append(options, option.WithResponseBodyInto(&res))
	_, err = client.CheckoutSessions.Preview(ctx, params, options...)
	if err != nil {
		return err
	}
	...
	format := cmd.Root().String("format")
	transform := cmd.Root().String("transform")
	return ShowJSON(os.Stdout, "checkout-sessions preview", obj, format, transform)
}
